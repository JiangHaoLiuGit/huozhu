<template>
	<view class="orderRate">
		...
		<view class="rateButton u-margin-top-60">
...
<script>
	...
	export default {
		...
		onLoad(option){
			// if(option.ids){
			// 	this.type = 2
				this.orderId = this.$store.state.ids
				console.log(this.orderId)
			// 	console.log(orderId)
			// }
			// uni.$on("ids", (data) => {
			// console.log("接收事件ids成功,data=", data);
		 //  });
		...
		methods: {
			...
			btnShu() {
				...
				ajax.dele(config.orderList_url + `/${this.orderId}/cancel`,data
				).then(res => {
					if(res.code == 0){
						console.log(res)
						this.show = false
