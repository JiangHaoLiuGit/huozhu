<template>
	<view class="page">
		<u-navbar back-text="" title="更换手机号" title-color="#ffffff" back-icon-color="#333" :background="{ background: '#0080FF' }" :borderBottom="false">
			...
			to2(){
				if(this.phoneCode != ""){
					ajax.get(config.phoneYan_url,{
					  ...
					  if(res.code == 0){
						  uni.navigateTo({
						  	url: "/pages/user/userDeitPhone/userDeitPhone"
						  })
					  }else{
						  this.show = true
						  this.showErr = "验证码不正确"
